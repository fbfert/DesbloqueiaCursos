...
const POR_LOTE = { n: 5, facil: 1, media: 3, dificil: 1 }
...
- Módulo: ${d.modulo} — **${d.peso} das 80 questões da prova**
...
Esta disciplina vale ${d.peso} questões: priorize o que mais cai e não tente esgotar a matéria. Densidade alta, escopo enxuto — o aluno tem 20 disciplinas para cobrir.
...
function promptLote(d, l) {
  ...
  return `Você vai escrever ${POR_LOTE.n} questões objetivas para o quiz de treino de **${d.modulo.replace(/^\d+\.\s*/, '')}** do curso "OAB 1ª Fase Completo" (curso 125), no padrão da FGV.
...
Exatamente **1 fácil, 3 médias e 1 difícil**. Os ${total} lotes juntos precisam fechar ${total * POR_LOTE.facil} fáceis, ${total * POR_LOTE.media} médias e ${total * POR_LOTE.dificil} difíceis — que é o que o sorteio exige para as três tentativas nunca repetirem questão nem completarem de outra faixa.
...
log(`aulas ${gravadas}/${filaAulas.length} · questões ${importadas}/${filaLotes.length * POR_LOTE.n}`)

return {
  aulasGravadas: gravadas,
  aulasTotal: filaAulas.length,
  questoesImportadas: importadas,
  questoesTotal: filaLotes.length * POR_LOTE.n,
