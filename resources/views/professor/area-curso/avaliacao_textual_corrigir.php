...
<section class="status-card">
    ...
    <?php if (!empty($entrega['imagens'])): ?>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:14px;">
            <?php foreach ($entrega['imagens'] as $img): ?>
                <a href="/professor/area-curso/conteudo/avaliacao/imagem?id=<?php echo (int) $img['id']; ?>" target="_blank" rel="noopener noreferrer" style="display:block;width:110px;height:110px;border-radius:10px;overflow:hidden;border:1px solid #d8dde6;">
                    <img src="/professor/area-curso/conteudo/avaliacao/imagem?id=<?php echo (int) $img['id']; ?>" alt="<?php echo Helpers::e((string) ($img['nome_original'] ?? 'Imagem enviada')); ?>" loading="lazy" style="width:100%;height:100%;object-fit:cover;display:block;">
                </a>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>

    <form method="post" action="/professor/area-curso/conteudo/avaliacao/corrigir" class="form-grid">
        <?php echo $csrfField; ?>
    ...
    <form method="post" action="/professor/area-curso/conteudo/avaliacao/liberar-reenvio" class="form-grid" style="margin-top:12px;">
        <?php echo $csrfField; ?>
        <input type="hidden" name="entrega_id" value="<?php echo (int) ($entrega['id'] ?? 0); ?>">
